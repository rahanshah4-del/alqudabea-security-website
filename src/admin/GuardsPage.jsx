...
import { Search, Download, Printer, X, Trash2, Edit3, Check, UserPlus, Shield } from 'lucide-react';
...
export default function GuardsPage() {
  ...
  const [form, setForm] = useState({ name: '', dept: 'Manned Guarding', position: '', nationality: 'Bahraini', joinDate: '', status: 'On Duty', cpr: 'Valid', visa: 'N/A', passport: 'Valid' });
  ...
  const openAdd = () => { setEditId(null); setForm({ name: '', dept: 'Manned Guarding', position: '', nationality: 'Bahraini', joinDate: '', status: 'On Duty', cpr: 'Valid', visa: 'N/A', passport: 'Valid' }); setShowForm(true); };
  const openEdit = (g) => { setEditId(g.id); setForm({ name: g.name, dept: g.dept, position: g.position, nationality: g.nationality, joinDate: g.joinDate, status: g.status, cpr: g.cpr, visa: g.visa, passport: g.passport }); setShowForm(true); };
  const handleSave = () => {
    if (!form.name.trim()) { return; }
    if (editId) { setGuards((prev) => prev.map((g) => g.id === editId ? { ...g, ...form } : g)); }
    else { const newId = `G-${String(guards.length + 1).padStart(3, '0')}`; setGuards((prev) => [...prev, { id: newId, ...form }]); }
    setShowForm(false);
  };
  const handleDelete = (id) => { setGuards((prev) => prev.filter((g) => g.id !== id)); };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-theme-muted">{guards.length} guards · {guards.filter((g) => g.status === 'On Duty').length} on duty · {guards.filter((g) => g.cpr === 'Expiring' || g.visa.includes('Expir') || g.passport === 'Expiring').length} need attention</p>
        </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-theme-muted">
        <table className="w-full text-left text-sm">
          <thead className="border-b border-theme-muted bg-surface-muted/40"><tr>
            <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-theme-muted">Guard</th><th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-theme-muted">Dept</th><th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-theme-muted">Position</th><th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-theme-muted hidden lg:table-cell">Nationality</th><th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-theme-muted hidden lg:table-cell">Joined</th><th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-theme-muted">Status</th><th className="px-4 py-3 font-mono text-[10px] uppercase tracking-wider text-theme-muted">CPR/Visa</th><th className="px-4 py-3" />
          </tr></thead>
          <tbody>{filtered.map((g) => (
            <tr key={g.id} className="border-b border-theme-muted transition-colors hover:bg-surface-muted/40">
              ...
              <td className="px-4 py-3 text-theme-secondary">{g.dept}</td><td className="px-4 py-3 text-theme-secondary">{g.position}</td>
              <td className="px-4 py-3 text-theme-secondary hidden lg:table-cell">{g.nationality}</td><td className="px-4 py-3 text-theme-muted hidden lg:table-cell">{g.joinDate}</td>
      ...
      {filtered.length === 0 && <div className="py-16 text-center"><Shield className="mx-auto h-10 w-10 text-theme-muted" /><p className="mt-3 text-sm text-theme-muted">No guards match your filters</p></div>}
