...
export default function VehiclesPage() {
  ...
  const handleSave = () => {
    ...
    if (editId) { setFleet((prev) => prev.map((v) => v.id === editId ? { ...v, ...form } : v)); }
    else { const newId = `VH-${String(fleet.length + 1).padStart(3, '0')}`; setFleet((prev) => [...prev, { id: newId, ...form }]); }
    setShowForm(false);
  };

  const handleDelete = (id) => { setFleet((prev) => prev.filter((v) => v.id !== id)); };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div><h1 className="font-sans text-2xl font-bold tracking-[-0.02em] text-theme-primary">Vehicle Management</h1><p className="mt-1 text-sm text-theme-muted">{fleet.length} vehicles · {activeCount} active · {maintCount} maintenance</p></div>
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        {[
          ...
          { icon: Shield, label: 'Insured', value: `${insuredCount}/${fleet.length}`, color: 'text-green-400', bg: 'bg-green-500/10' },
        ].map((s) => (
