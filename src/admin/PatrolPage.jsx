...
export default function PatrolPage() {
  ...
  useEffect(() => {
    Promise.all([getCollection('patrols'), getCollection('missed_checkpoints')])
      .then(([r, m]) => { setRoutes(r); setMissed(m); setLoading(false); })
  ...
  const handleSave = async () => {
    ...
    if (editId) {
      await updateDocument('patrols', editId, form);
      setRoutes((prev) => prev.map((r) => r.id === editId ? { ...r, ...form, completed: r.completed } : r));
    } else {
      const newId = await addDocument('patrols', { ...form, completed: 0, time: 'Just started' });
      if (newId) { setRoutes((prev) => [...prev, { id: newId, ...form, completed: 0, time: 'Just started' }]); }
    }
    setShowForm(false);
  };

  const handleDelete = async (id) => { await deleteDocument('patrols', id); setRoutes((prev) => prev.filter((r) => r.id !== id)); };
  const handleQr = () => { setQrGenerated(true); setTimeout(() => setQrGenerated(false), 3000); };
  const resolveMissed = async (idx) => {
    const item = missed[idx];
    if (item?.id) { await deleteDocument('missed_checkpoints', item.id); }
    setMissed((prev) => prev.filter((_, i) => i !== idx));
  };
