...
import { IncidentsAPI } from '@/firebase/services';
...
export default function IncidentsPage() {
  ...
  const handleSave = async () => {
    ...
    if (editId) {
      ...
      setIncidents((prev) => prev.map((i) => i.id === editId ? { ...i, ...form } : i));
    } else {
      const newId = await IncidentsAPI.add(form);
      if (newId) {
        const now = new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
        setIncidents((prev) => [{ id: newId, ...form, time: now }, ...prev]);
      }
    }
    setShowForm(false);
  };
  ...
  const handleDelete = async (id) => {
    await IncidentsAPI.update(id, { status: 'Deleted' }).catch(() => {});
    setIncidents((prev) => prev.filter((i) => i.id !== id));
  };
  const toggleStatus = async (id) => {
    const inc = incidents.find((i) => i.id === id);
    const newStatus = inc.status === 'Resolved' ? 'Investigating' : 'Resolved';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div><h1 className="font-sans text-2xl font-bold tracking-[-0.02em] text-theme-primary">Incident Management</h1><p className="mt-1 text-sm text-theme-muted">{loading ? 'Loading...' : `${incidents.length} incidents · ${incidents.filter((i) => i.status !== 'Resolved').length} open`}</p></div>
      ...
      {loading ? (
        <div className="py-16 text-center"><div className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-accent-500 border-t-transparent" /><p className="mt-4 text-sm text-theme-muted">Loading incidents...</p></div>
      ) : incidents.length === 0 ? (
        <div className="py-16 text-center"><AlertTriangle className="mx-auto h-10 w-10 text-theme-muted" /><p className="mt-3 text-sm text-theme-muted">No incidents reported yet. Click "Report Incident" to log one.</p></div>
      ) : (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-theme-muted">{inc.site} · {inc.officer} · <Clock className="inline h-3 w-3" /> {inc.time || inc.createdAt?.toDate?.().toLocaleTimeString() || '—'}</p>
                  </div>
                </div>
              </div>
